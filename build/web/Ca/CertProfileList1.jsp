<%-- 
    ...
    Created on : May 3, 2018, 11:01:40 AM
    Author     : THANH-PC
--%>
                    ...
                    <div class="">
                        <div class="row">
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                <div class="x_panel">
                                    <div class="x_title">
                                        <h2><i class="fa fa-list-ul"></i> <script>document.write(certprofile_table_list);</script></h2>
                                    ...
                                    <%
                                        ...
                                        try {
                                            ...
                                            if (session.getAttribute("SessRefreshCertProfile") != null) {
                                                session.setAttribute("SessRefreshCertProfile", null);
                                                int intCount = 0;//db.S_BO_CERTIFICATION_PROFILE_TOTAL();
                                                if (intCount > 0) {
                                                    if (session.getAttribute("IPageNoSessListCertProfile") != null) {
                                                        String sPage = (String) session.getAttribute("IPageNoSessListCertProfile");
                                                        iPagNo = Integer.parseInt(sPage);
                                                    }
                                                    if (session.getAttribute("ISwRwsSessListCertProfile") != null) {
                                                        String sSumPage = (String) session.getAttribute("ISwRwsSessListCertProfile");
                                                        iSwRws = Integer.parseInt(sSumPage);
                                                    }
                                                    if (session.getAttribute("NumPageSessListCertProfile") != null) {
                                                        String sNoPage = (String) session.getAttribute("NumPageSessListCertProfile");
                                                        iPaNoSS = Integer.parseInt(sNoPage);
                                                    }
                                                    session.setAttribute("IPageNoSessListCertProfile", String.valueOf(iPagNo));
                                                    session.setAttribute("ISwRwsSessListCertProfile", String.valueOf(iSwRws));
                                                    //db.S_BO_CERTIFICATION_PROFILE_LIST(sessLanguageGlobal, iPagNo, iSwRws, rsPgin);
                                                }
                                                iTotRslts = intCount;
                                            }
                                            else
                                            {
                                                session.setAttribute("NumPageSessListCertProfile", null);
                                                session.setAttribute("IPageNoSessListCertProfile", null);
                                                session.setAttribute("ISwRwsSessListCertProfile", null);
                                                if (hasPaging != true) {
                                                    session.setAttribute("CountCertProfileList", null);
                                                }
                                                int intCount = 0;
                                                if ((session.getAttribute("CountCertProfileList")) == null) {
                                                    intCount = 0;//db.S_BO_CERTIFICATION_PROFILE_TOTAL();
                                                    session.setAttribute("CountCertProfileList", String.valueOf(intCount));
                                                } else {
                                                    String sCountTemp = (String) session.getAttribute("CountCertProfileList");
