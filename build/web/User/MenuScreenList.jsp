<%-- 
    Document   : MenuScreenList
    Created on : Nov 24, 2017, 11:22:47 AM
    Author     : THANH-PC
--%>
...
<html>
    <head>
        ...
        <script language="javascript">
            changeFavicon("../");
            document.title = menusc_title_list;
            $(document).ready(function () {
                localStorage.setItem("LOCAL_PARAM_MENUSCREENLIST", null);
            });
            function popupEdit(id)
            {
                ...
                $(".loading-gif").show();
                $('#contentEdit').empty();
                $('#contentEdit').load('MenuScreenEdit.jsp', {id:id}, function () {
                    $('#idX_Panel_Edit').css("display", "");
                    goToByScroll("contentEdit");
                });
                $(".loading-gif").hide();
                $('#over').remove();
//                localStorage.setItem("LOCAL_PARAM_MENUSCREENLIST", id);
//                window.location = 'MenuScreenEdit.jsp?id=' + id;
            }
            function addForm()
            {
                window.location = 'MenuScreenNew.jsp';
            }
        </script>
    </head>
    <body class="nav-md">
        ...
        <div class="container body">
            <div class="main_container">
                ...
                <div class="right_col" role="main">
                            ...
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                <div class="x_panel">
                                    <div class="x_title" style="border-bottom: 0 solid #E6E9ED;margin-bottom: 0px;">
                                        <h2><i class="fa fa-list-ul"></i> <script>document.write(menusc_title_table);</script></h2>
                                    ...
                                    <%
                                        MENULINK[][] rsPgin = new MENULINK[1][];
                                        try {
                                            ...
                                            if (hasPaging != true) {
                                                session.setAttribute("CountListScreen", null);
                                            }
                                            int ss = 0;
                                            String sessLanguageGlobal = session.getAttribute("sessVN").toString();
                                            if (session.getAttribute("SessRefreshMenuScreen") != null) {
                                                session.setAttribute("SessRefreshMenuScreen", null);
                                                if ((session.getAttribute("CountListScreen")) == null) {
                                                    ss = db.S_BO_URI_TOTAL();
                                                    session.setAttribute("CountListScreen", String.valueOf(ss));
                                                } else {
                                                    String sCount = (String) session.getAttribute("CountListScreen");
                                                    ss = Integer.parseInt(sCount);
                                                    session.setAttribute("CountListScreen", String.valueOf(ss));
                                                }
                                                if (ss > 0) {
                                                    if (session.getAttribute("IPageNoSessListScreen") != null) {
                                                        String sPage = (String) session.getAttribute("IPageNoSessListScreen");
                                                        iPagNo = Integer.parseInt(sPage);
                                                    }
                                                    if (session.getAttribute("ISwRwsSessListScreen") != null) {
                                                        String sSumPage = (String) session.getAttribute("ISwRwsSessListScreen");
                                                        iSwRws = Integer.parseInt(sSumPage);
                                                    }
                                                    if (session.getAttribute("NumPageSessListScreen") != null) {
                                                        String sNoPage = (String) session.getAttribute("NumPageSessListScreen");
                                                        iPaNoSS = Integer.parseInt(sNoPage);
                                                    }
                                                    db.S_BO_URI_LIST(iPagNo, iSwRws, sessLanguageGlobal, rsPgin);
                                                    session.setAttribute("IPageNoSessListScreen", String.valueOf(iPagNo));
                                                    session.setAttribute("ISwRwsSessListScreen", String.valueOf(iSwRws));
                                                }
                                            } else {
                                                session.setAttribute("NumPageSessListScreen", null);
                                                session.setAttribute("IPageNoSessListScreen", null);
                                                session.setAttribute("ISwRwsSessListScreen", null);
                                                if ((session.getAttribute("CountListScreen")) == null) {
                                                    ss = db.S_BO_URI_TOTAL();
                                                    session.setAttribute("CountListScreen", String.valueOf(ss));
                                                } else {
                                        ...
                                        <div class="x_content">
                                            ...
                                            <div class="table-responsive">
                                                <table id="idTableList" class="table table-bordered table-striped projects">
                                                    ...
                                                    <th><script>document.write(menusc_fm_name);</script></th>
                                                    <th><script>document.write(menusc_fm_nameparent);</script></th>
                                                    <th><script>document.write(menusc_fm_url);</script></th>
                                                    ...
                                                    <tbody>
                                                        <%
                                                            ...
                                                            session.setAttribute("NumPageSessListScreen", String.valueOf(iPaNoSS));
                                                            if(rsPgin[0].length > 0) {
                                                                ...
                                                                String strParentName = EscapeUtils.CheckTextNull(rsPgin[0][i].PARENTLINK_REMARK);
                                                                String strURL = EscapeUtils.CheckTextNull(rsPgin[0][i].LINKURL);
                    ...
                    <script>
                        $(document).ready(function () {
                            if(localStorage.getItem("EDIT_MENUSCREEN") !== null && localStorage.getItem("EDIT_MENUSCREEN") !== "null")
                            {
                                var vIDEDIT_MENUSCREEN = localStorage.getItem("EDIT_MENUSCREEN");
                                localStorage.setItem("EDIT_MENUSCREEN", null);
                                popupEdit(vIDEDIT_MENUSCREEN);
                            }
                        });
                    </script>
                </div>
