<%-- 
    Document   : ProfileAccessList
    Created on : Dec 18, 2019, 1:38:45 PM
    Author     : USER
--%>
    ...
    <head>
        ...
        <script language="javascript">
            ...
            $(document).ready(function () {
                localStorage.setItem("LOCAL_PARAM_BRANCHROLE_LIST", null);
            });
            function popupEdit(id)
            {
                $('body').append('<div id="over"></div>');
                $(".loading-gif").show();
                $('#contentEdit').load('ProfileAccessEdit.jsp', {id:id}, function () {
            ...
            function addForm()
            {
                window.location = 'ProfileAccessAdd.jsp';
            }
        </script>
    </head>
    <body class="nav-md">
                            ...
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                <div class="x_panel">
                                    <div class="x_title" style="border-bottom: 0 solid #E6E9ED;margin-bottom: 0px;">
                                        <h2><i class="fa fa-list-ul"></i> <script>document.write(profileaccss_title_table);</script></h2>
                                    ...
                                    <%
                                        BRANCH_ROLE[][] rsPgin = new BRANCH_ROLE[1][];
                                        try {
                                            ...
                                            if (session.getAttribute("SessRefreshBranchRole") != null) {
                                                session.setAttribute("SessRefreshBranchRole", null);
                                                int ss = db.S_BO_BRANCH_ROLE_TOTAL();
                                                if (ss > 0) {
                                                    if (session.getAttribute("IPageNoSessListBranchRole") != null) {
                                                        String sPage = (String) session.getAttribute("IPageNoSessListBranchRole");
                                                        iPagNo = Integer.parseInt(sPage);
                                                    }
                                                    if (session.getAttribute("ISwRwsSessListBranchRole") != null) {
                                                        String sSumPage = (String) session.getAttribute("ISwRwsSessListBranchRole");
                                                        iSwRws = Integer.parseInt(sSumPage);
                                                    }
                                                    if (session.getAttribute("NumPageSessListBranchRole") != null) {
                                                        String sNoPage = (String) session.getAttribute("NumPageSessListBranchRole");
                                                        iPaNoSS = Integer.parseInt(sNoPage);
                                                    }
                                                    session.setAttribute("IPageNoSessListBranchRole", String.valueOf(iPagNo));
                                                    session.setAttribute("ISwRwsSessListBranchRole", String.valueOf(iSwRws));
                                                    db.S_BO_BRANCH_ROLE_LIST(sessLanguageGlobal, iPagNo, iSwRws, rsPgin);
                                                }
                                                iTotRslts = ss;
                                            }
                                            else
                                            {
                                                session.setAttribute("NumPageSessListBranchRole", null);
                                                session.setAttribute("IPageNoSessListBranchRole", null);
                                                session.setAttribute("ISwRwsSessListBranchRole", null);
                                                if (hasPaging != true) {
                                                    session.setAttribute("CountListBranchRole", null);
                                                }
                                                int ss = 0;
                                                if ((session.getAttribute("CountListBranchRole")) == null) {
                                                    ss = db.S_BO_BRANCH_ROLE_TOTAL();
                                                    session.setAttribute("CountListBranchRole", String.valueOf(ss));
                                                } else {
                                                    String sCount = (String) session.getAttribute("CountListBranchRole");
                                        ...
                                        <div class="x_content">
                                            ...
                                            <div class="table-responsive">
                                                <table id="idTableList" class="table table-bordered table-striped projects">
                                                    ...
                                                    <th><script>document.write(profileaccss_fm_code);</script></th>
                                                    ...
                                                    <tbody>
                                                        <%
                                                            ...
                                                            session.setAttribute("NumPageSessListBranchRole", String.valueOf(iPaNoSS));
