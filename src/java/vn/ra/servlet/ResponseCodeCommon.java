 ...
 * @author THANH-PC
 */
public class ResponseCodeCommon extends HttpServlet {

    private static final org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(ResponseCodeCommon.class);
            ...
            try {
                ...
                if (sOutInner == 1) {
                    ...
                    if (null != idParam) {
                        switch (idParam) {
                            case "editresponse": {
                                //<editor-fold defaultstate="collapsed" desc="editresponse">
                                ...
                                if (anticsrf != null && anticsrf.equals(request.getSession().getAttribute("anticsrf"))) {
                                    ...
                                    String param1 = com.S_BO_RESPONSE_CODE_UPDATE(Integer.parseInt(ids),
                                            ActiveFlag, EscapeUtils.escapeHtml(pName),
                                            EscapeUtils.escapeHtml(Remark_EN),
                                            EscapeUtils.escapeHtml(Remark), loginUID);
                                    if ("0".equals(param1)) {
                                        request.getSession(false).setAttribute("SessRefreshResponse", "1");
                                        strView = "0#0";
                                    } else {
                            ...
                            case "addresponse": {
                                //<editor-fold defaultstate="collapsed" desc="addresponse">
                                String anticsrf = request.getParameter("CsrfToken");
                                if (anticsrf != null && anticsrf.equals(request.getSession().getAttribute("anticsrf"))) {
                                    String pName = request.getParameter("citycode");
                                    String Remark = request.getParameter("Remark");
                                    String Remark_EN = request.getParameter("Remark_EN");
                                    String param1 = com.S_BO_RESPONSE_CODE_INSERT(EscapeUtils.escapeHtml(pName),
